...
export function GlossyButton({ children, className, ...props }: GlossyButtonProps) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className={cn(
        "glossy-button relative overflow-hidden px-8 py-4 rounded-full font-display font-bold text-lg text-green-900 tracking-wide transition-all duration-200",
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/70 to-transparent opacity-60 rounded-t-full pointer-events-none" />
